    ...
    <>
      <div className={styles.agentFormContainer}>
        ...
        <div className={styles.agentFormSection} style={{ padding: '16px', backgroundColor: '#EFF6FF', borderRadius: '8px', marginBottom: '16px' }}>
          ...
          <div className={styles.radioGroup} style={{ flexWrap: 'wrap' }}>
            {MODEL_PRESETS.map((p) => (
              <button
                key={p.key}
                className={`${styles.radioBtn} ${form.provider === p.key ? styles.activeFull : ''}`}
                onClick={() => update('provider', p.key) || update('apiBaseUrl', p.url || form.apiBaseUrl) || update('modelName', p.model || form.modelName)}
              >{p.name}</button>
            ))}
          </div>
        </div>
        ...
        <div className={styles.agentFormSection} style={{ padding: '16px', backgroundColor: '#F5F3FF', borderRadius: '8px', marginBottom: '16px' }}>
          ...
          <input type="text" className={styles.agentFormInput} value={form.modelName} onChange={(e) => update('modelName', e.target.value)} placeholder="gpt-4o / gemini-2.5-pro / deepseek-chat" />
        </div>

        {/* 上下文记忆轮次 */}
        <div className={styles.agentFormSection}>
          <span className={styles.agentFormLabel}>上下文记忆轮次: {form.contextRounds}</span>
          <input type="range" min="1" max="100" step="1" className={styles.kbRange} value={form.contextRounds} onChange={(e) => update('contextRounds', parseInt(e.target.value))} />
        ...
        {form.kbSource !== 'none' && (
          <>
            ...
            <div className={styles.agentFormSection} style={{ padding: '16px', backgroundColor: '#FDF2F8', borderRadius: '8px', marginBottom: '16px' }}>
              ...
              <input type="password" className={styles.agentFormInput} value={form.kbId} onChange={(e) => update('kbId', e.target.value)} placeholder="请输入对应的库 API Key..." />
            </div>
          </>
        )}
        ...
        {/* ========== 回复内容设置 ========== */}
        <div className={styles.sectionBanner}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="16" height="16" style={{ color: 'var(--color-primary)' }}><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>
          <span>回复内容设置</span>
        </div>

        <div className={styles.agentFormSection} style={{ padding: '16px', backgroundColor: '#ECFCCB', borderRadius: '8px', marginBottom: '16px' }}>
          <span className={styles.agentFormLabel} style={{ color: '#84CC16' }}>最大回复长度: {form.maxReplyLength} 字</span>
          <input type="range" min="50" max="500" step="50" className={styles.kbRange} value={form.maxReplyLength} onChange={(e) => update('maxReplyLength', parseInt(e.target.value))} />
        </div>

        <div className={styles.toggleRow}>
          <span className={styles.toggleLabel}>允许发送图片</span>
          <div className={styles.toggleSwitch}>
            <input type="checkbox" id="toggleImg" checked={form.allowImages} onChange={(e) => update('allowImages', e.target.checked)} />
        ...
        <div className={styles.toggleRow}>
          ...
          <div className={styles.toggleSwitch}>
            <input type="checkbox" id="toggleFile" checked={form.allowFiles} onChange={(e) => update('allowFiles', e.target.checked)} />
        ...
        {form.segmentEnabled && (
          <>
            <div className={styles.agentFormSection} style={{ padding: '16px', backgroundColor: '#F3F4F6', borderRadius: '8px', marginBottom: '8px' }}>
              ...
              <input type="range" min="2" max="5" step="1" className={styles.kbRange} value={form.segmentCount} onChange={(e) => update('segmentCount', parseInt(e.target.value))} />
            </div>
            <div className={styles.agentFormSection} style={{ padding: '16px', backgroundColor: '#FDF4FF', borderRadius: '8px', marginBottom: '8px' }}>
              ...
              <input type="range" min="30" max="200" step="10" className={styles.kbRange} value={form.segmentTriggerChars} onChange={(e) => update('segmentTriggerChars', parseInt(e.target.value))} />
