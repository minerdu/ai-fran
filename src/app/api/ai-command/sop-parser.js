 ...
 * @param {object[]} targetLeads - 目标线索列表（含 id, name）
 * @param {object} meta - 元数据
 * @param {string} meta.command - 原始招商指令
 * @param {string} meta.intent - LLM 解析的意图
 * @param {boolean} meta.needApproval - 是否全部需要人工审批
 * @returns {Promise<{tasks: object[], summary: string}>}
 */
export async function parseSopScheduleToTasks(sopSchedule, targetLeads, meta = {}) {
  ...
  const summary = `已为 ${targetLeads.length} 位线索创建 ${steps.length} 步 SOP 工作流，共 ${tasksCreated.length} 条任务。`;
