...
/**
 * GET /api/youzan/sync — 获取招商线索同步状态和历史
 */
...
/**
 * POST /api/youzan/sync — 手动触发外部渠道线索同步
 */
export async function POST() {
  ...
  try {
    ...
    if (!config || !config.appId || !config.appSecret) {
      ...
      return NextResponse.json({
        success: false,
        message: '请先配置渠道接入的 App ID 和 App Secret',
      }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ...
      message: result.success
        ? `同步完成，获取 ${result.totalFetched} 条线索，新增 ${result.totalCreated}，更新 ${result.totalUpdated}`
        : `同步完成但有异常：${result.errors.length} 条`,
      ...result,
    });
  } catch (error) {
    isSyncing = false;
