...
/**
 * GET /api/youzan — 获取 CRM / 渠道接入配置（脱敏）
 */
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: '获取接入配置失败' }, { status: 500 });
  }
}

/**
 * PUT /api/youzan — 保存 CRM / 渠道接入配置
 */
export async function PUT(request) {
  try {
    ...
    return NextResponse.json({ error: '保存接入配置失败' }, { status: 500 });
  }
}

/**
 * POST /api/youzan — 测试渠道 API 连接
 */
