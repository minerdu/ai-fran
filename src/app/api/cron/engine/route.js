 ...
 *
 * 增强点：
 * 1. 执行前动态预检 — 调 LLM 判断"该线索当前是否适合发送原定消息"
...
async function precheckTask(task, config) {
  ...
  if (!customer) {
    return { shouldExecute: true, reason: '无线索信息，跳过预检' };
  }
...
请回答 "EXECUTE" 如果适合执行，或 "SKIP:原因" 如果不适合（如已签约线索仍收到早期培育消息）。只回答这一行，不要其他内容。`;
...
async function markTaskSkipped(prisma, task, reason) {
    ...
    for (const task of tasksToExecute) {
      try {
        ...
        if (!precheck.shouldExecute) {
          ...
          await markTaskSkipped(prisma, task, `AI预检跳过: ${precheck.reason}`);
          skippedIds.push(task.id);
          continue;
        }

        if (!task.customer || !task.customer.wechatId) {
          const reason = '线索缺少企业微信标识，自动跳过执行';
          console.log(`[Cron Engine] Skipping task ${task.id}: ${reason}`);
          await markTaskSkipped(prisma, task, reason);
          skippedIds.push(task.id);
          continue;
        }
        ...
        try {
          ...
          // 如果线索当前仍在线索池，则升级为已跟进
          if (['new', 'pool', 'silent'].includes(task.customer.lifecycleStatus)) {
            updateData.lifecycleStatus = 'qualified';
          }
          await prisma.customer.update({
            where: { id: task.customer.id },
            data: updateData,
          });
          console.log(`[Cron Engine] Lead ${task.customer.name} state updated: silentDays=0, lastInteraction=now`);
        } catch (custErr) {
          console.warn(`[Cron Engine] Failed to update lead state for ${task.customer.id}:`, custErr.message);
        }
