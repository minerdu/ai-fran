...
export default function ApprovalsPage() {
  ...
  useEffect(() => {
    loadApprovals();
    // Auto-refresh when tab becomes visible (cross-tab sync for AI command results)
  ...
  const pendingFilteredIds = useMemo(() =>
    filteredItems.filter(i => i.status === 'pending').map(i => i.id),
  [filteredItems]);
  ...
  const summaryCards = [
    ...
    { label: '今日处理', value: items.filter(isProcessedToday).length, accent: '#2563eb' },
  ];
          ...
          filteredItems.map((approval, index) => {
            ...
            return (
              ...
              >
                ...
                {/* History Bar for processed items */}
