...
import { normalizeLeadTask } from '@/lib/leadAdapters';
import { getOptimizationTaskMeta, isOptimizationTask, summarizeOptimizationTasks } from '@/lib/optimizationTaskMeta';
...
export default function TasksPage() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const normalized = Array.isArray(data) ? data.map(normalizeLeadTask) : [];
      setTasks(normalized);
    } catch (e) {
  ...
  const optimizationSummary = useMemo(
    () => summarizeOptimizationTasks(filteredTasks),
    [filteredTasks]
  );
  ...
  const handleBatchApprove = async () => {
    ...
    if (confirm(`确认批量通过 ${selectedTaskIds.size} 个招商任务？`)) {
      ...
      <div className={styles.taskList}>
        {optimizationSummary.length > 0 && (
          <section className={styles.optimizationPanel}>
            <div className={styles.optimizationPanelHead}>
              <div>
                <strong className={styles.optimizationPanelTitle}>优化建议已进入当前任务队列</strong>
                <p className={styles.optimizationPanelDesc}>
                  当前标签下共有 {filteredTasks.filter(isOptimizationTask).length} 条任务来自 AI 优化建议，已按执行类型归类。
                </p>
              </div>
            </div>
            <div className={styles.optimizationSummaryRow}>
              {optimizationSummary.map((item) => (
                <a key={item.key} href={item.href} className={styles.optimizationSummaryChip}>
        ...
        {isLoading ? (
          ...
          filteredTasks.map((task, index) => (
            (() => {
              const optimizationMeta = isOptimizationTask(task) ? getOptimizationTaskMeta(task) : null;
              ...
              return (
                ...
                >
                        ...
                        {optimizationMeta ? (
                          <div className={styles.originTagRow}>
                            <span
                              ...
                              style={{ color: optimizationMeta.accent, background: optimizationMeta.bg }}
                            >
                              {optimizationMeta.icon} 来自优化建议
                            </span>
                            <span
                              className={styles.originBadge}
                              style={{ color: optimizationMeta.accent, background: optimizationMeta.bg }}
                  ...
                  <div className={styles.aiReason}>
                    <span className={styles.aiReasonIcon}>{optimizationMeta?.icon || '🤖'}</span>
                    <div className={styles.aiReasonBody}>
                      <p className={styles.aiReasonText}>{task.triggerReason || '无分析说明'}</p>
                      {optimizationMeta ? (
                        <Link
                          href={optimizationMeta.href}
                          className={styles.aiReasonLink}
                          onClick={(event) => event.stopPropagation()}
                        >
                          查看 {optimizationMeta.label} 入口
                        </Link>
                      ) : null}
                    </div>
                  </div>
                  ...
                  {optimizationMeta ? (
                    <div className={styles.originStrip}>
                      <span className={styles.originStripText}>
                        {optimizationMeta.icon} 当前任务由「{optimizationMeta.label}」优化动作生成
                      </span>
                      <Link
                        href={optimizationMeta.href}
                        className={styles.originStripLink}
                        onClick={(event) => event.stopPropagation()}
                      >
                        打开入口
                      </Link>
          ...
          <div className={styles.drawer} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.drawerBody}>
              <div className={styles.drawerSection}>
                <label className={styles.drawerLabel}>线索</label>
                <div className={styles.drawerCustomer}>
                  <span className={styles.drawerAvatar}>{selectedTask.leadName.slice(-2)}</span>
              ...
              <div className={styles.drawerSection}>
                ...
                <div className={styles.drawerAiBox}>
                  ...
                  {isOptimizationTask(selectedTask) ? (
                    <Link
                      href={getOptimizationTaskMeta(selectedTask).href}
                      className={styles.drawerOriginLink}
                    >
                      {getOptimizationTaskMeta(selectedTask).icon} 该任务来自 {getOptimizationTaskMeta(selectedTask).label} 优化动作
                    </Link>
                  ) : null}
                </div>
              </div>
